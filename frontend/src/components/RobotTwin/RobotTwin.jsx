...
function CameraController({ anomalyJoint, isResetting, onResetComplete }) {
  ...
  const defaultPos = useRef(new THREE.Vector3(1.1, 0.85, 1.1));
...
export default function RobotTwin({ compact = false, onJointClick }) {
  ...
  return (
    <div className={`robot-twin-container ${compact ? 'compact' : ''}`}>
      {/* Live Badge */}
      <div className="twin-badge-live">
        <span className="twin-pulse-dot" />
        <span>{connected ? 'Digital Twin • Live' : 'Digital Twin • Standby'}</span>
      </div>
      ...
      {/* Anomaly Callout */}
      {anomalyJoint !== null && (
        <div className="twin-anomaly-focus-banner">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
            <circle cx="12" cy="12" r="10" />
            <line x1="12" y1="8" x2="12" y2="12" />
            <line x1="12" y1="16" x2="12.01" y2="16" />
          </svg>
          <span>Focusing on {JOINT_NAMES[anomalyJoint]} anomaly</span>
        </div>
      )}
      ...
      >
        <Canvas
          ...
          camera={{ position: [1.1, 0.85, 1.1], fov: 42 }}
          dpr={[1, 2]}
          shadows
        >
          {/* Lighting */}
          <ambientLight intensity={0.65} color="#FFFBF7" />
          <hemisphereLight
            skyColor="#FFF9F2"
            groundColor="#8A8178"
            intensity={0.4}
